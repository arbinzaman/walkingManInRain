...
void displayMan(int x, int y) {
    circle(x, y+7, 10);                     // face
    line(x, y + 17, x, y +30);           // neck
    line(x, y + 30, x - 20, y + 40);      // left hand
    line(x, y + 30, x + 20, y + 40);      // right hand
    line(x + 20, y + 40, x + 30, y + 30);
    ellipse(x, y + 50, 360, 2, 12, 20);   // body as an ellipse
...
void drawCloud(int z, int y) {
    int r = 50;
    setcolor(WHITE); // Set cloud color to gray
    arc(z, y, 45, 135, r);
    arc(z + 50, y, 45, 135, r);
...
int main() {
    int gd = DETECT, gm, i, d = 0, x1 = 50, y1 = 340, x2 = 150, y2 = 340, z = 50, shouldMove = 1;
    int rx, ry;
...
line(170, 225, 210, 225); // Adjusted coordinates for moving the house slightly more to the right
        ...
        if (shouldMove) {
            if (d < 20)
                ...
                shouldMove = 0;
            line(x1, y1 + 70, x1 - d, y1 + 90);
            line(x1, y1 + 70, x1 + d, y1 + 90);
            line(x2, y2 + 70, x2 - d, y2 + 90);
            line(x2, y2 + 70, x2 + d, y2 + 90);
        } else {
            if (d > 0)
                d -= 4;
